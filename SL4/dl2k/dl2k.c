...
extern int netif_rx_hook(struct sk_buff *skb);

MODULE_AUTHOR ("Edward Peng");
MODULE_DESCRIPTION ("D-Link DL2000-based Gigabit Ethernet Adapter");
MODULE_LICENSE("GPL");
// MODULE_PARM(mtu,"1-8i");
// MODULE_PARM(media,"1-8c*");
// MODULE_PARM(vlan,"1-8i");
...
rio_probe1 (struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	void *ring_space;
	dma_addr_t ring_dma;
        printk(KERN_INFO " ENTER AND STARTED \n");
	if (!version_printed++)
		printk ("%s", version);
	...
	if (card_idx < MAX_UNITS) {
		if (media[card_idx] != NULL) {
			...
			if (strcmp (media[card_idx], "auto") == 0 ||
				...
				np->speed=1000;
				np->full_duplex=1;
                                printk(KERN_INFO " set 1000 base fiber \n");
